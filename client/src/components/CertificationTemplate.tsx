...
const CertificationTemplate = ({ id, thumbnail = false }: { id: string, thumbnail?: boolean }) => {
  ...
  if (!certification || !certification.student || !certification.course) {
    return (
      <div className="certificate-error" style={{ textAlign: 'center', color: 'red', padding: '2rem' }}>
  ...
  return (
    <div
      ...
      id={`certificate-${id}`}
      ref={certRef}
      style={thumbnail ? { width: 220, minWidth: 180, maxWidth: 240, height: 140, minHeight: 120, maxHeight: 180, padding: 8, fontSize: 12, boxShadow: "0 1px 4px #ccc" } : {}}
    >
      <div className="certificate-content" style={thumbnail ? { padding: 4, fontSize: 12 } : {}}>
        <h1 style={thumbnail ? { fontSize: 16, margin: 0 } : {}}>شهادة حضور</h1>
        <p style={thumbnail ? { fontSize: 10, margin: 0 } : {}}>تشهد منصّة علّم بسعادة أن الأستاذة:</p>
        <h2 style={thumbnail ? { fontSize: 13, margin: 0 } : {}}>{certification.student.fullName}</h2>
        <p style={thumbnail ? { fontSize: 10, margin: 0 } : {}}>
          قد حضرت ورشة:
          <span style={thumbnail ? { fontSize: 11 } : {}}>{certification.course.title}</span>
        </p>
        <p style={thumbnail ? { fontSize: 10, margin: 0 } : {}}>
      ...
      {!hideButton && !thumbnail && (
        <button className="get-certificate-btn" style={{ marginTop: 24 }} onClick={handleDownloadPDF}>
          تحميل الشهادة PDF
        </button>
      )}
    </div>
